...
def precise_dist_share_mem(
    ...
    num, _ = feat.shape
    num_process = min(num, num_process)
    num_per_proc = math.ceil(num / num_process)

    # Numerical stability when num is close tu num_process
    if num_per_proc * num_process - num >= num_per_proc:
        num_process -= 1
